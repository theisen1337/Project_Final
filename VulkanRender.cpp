...
VulkanView view = *Initialization::getView();
...
{
	QueueFamilyIndices queueFamilyIndices = VulkanView::findQueueFamilies(view.getPhysicalDevice());
...
{
	uint32_t imageIndex;
	vkAcquireNextImageKHR(view.getDevice(), view.getSwapChain(), std::numeric_limits<uint64_t>::max(), imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);
	...
	vkQueuePresentKHR(view.getPresentQueue(), &presentInfo);
}
